...
function submit(value: string) {
  fireEvent.change(screen.getByLabelText("Package number"), { target: { value } });
  fireEvent.submit(screen.getByRole("button", { name: "Search" }).closest("form")!);
}
function upload() {
  fireEvent.change(screen.getByLabelText("Upload QR Image"), { target: { files: [new File(["image"], "demo.png", { type: "image/png" })] } });
}
describe("package lookup UI", () => {
  it("trims manual entry and displays the result using form submission", async () => {
    ...
    expect(await screen.findByRole("article", { name: "Package 800001" })).toBeTruthy();
    fireEvent.click(screen.getByRole("button", { name: "Enter Another Number" }));
    expect(screen.queryByRole("article")).toBeNull();
    expect(document.activeElement).toBe(screen.getByLabelText("Package number"));
  });
  it("rejects manual invalid input before calling the service", async () => {
    render(<ScanPackage />); submit("invalid");
    expect(await screen.findByRole("alert")).toHaveProperty("textContent", expect.stringContaining("six-digit"));
  ...
  it("handles a valid unknown token after image decoding", async () => {
    ...
    expect(await screen.findByRole("alert")).toHaveProperty("textContent", expect.stringContaining("No package found"));
  ...
  it("shows the same information through the QR route", async () => {
    ...
    render(<ScanPackage />); upload();
    expect(await screen.findByRole("article", { name: "Package 800001" })).toHaveProperty("textContent", expect.stringContaining("Demo test kit"));
  });
  it("disables lookup controls while pending and reports service-style failures", async () => {
    ...
    expect(screen.getByLabelText("Package number")).toHaveProperty("disabled", true);
    ...
    await waitFor(() => expect(screen.getByRole("button", { name: "Search" })).toHaveProperty("disabled", false));
  });
